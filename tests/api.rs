...
//! tests pin the argv and cwd the real binaries would receive. The stub
//! `ecp` answers `admin mcp tools --format json` with the captured fixture.
...
const TOOLS_FIXTURE: &str = concat!(env!("CARGO_MANIFEST_DIR"), "/tests/fixtures/tools.json");
...
case "$1" in
  ...
  rev-parse) echo abc1234 ;;
    ...
    let state = AppState::new(
        load_tools(&ecp).expect("stub ecp serves the tools fixture"),
        Arc::new(store),
        runner,
