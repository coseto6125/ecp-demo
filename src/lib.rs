...
//! build. The tool list, its JSON schemas and the argv rules come from
//! `ecp admin mcp tools --format json` at startup, so the page shows exactly
//! the surface an agent gets and needs no Rust dependency on ecp itself.
