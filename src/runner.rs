...
use crate::tools::{build_argv, reserved_token, DemoTool};
use serde::Serialize;
...
impl Runner {
    ...
    /// Run `ecp <sub> [--repo <checkout>] <argv>` inside `checkout`.
    pub async fn run(
        &self,
        tool: &DemoTool,
        checkout: &Path,
        ...
        let caller_argv = build_argv(&tool.inner, args).map_err(RunError::BadArgs)?;
        ...
        cmd.args(&argv).current_dir(checkout);
